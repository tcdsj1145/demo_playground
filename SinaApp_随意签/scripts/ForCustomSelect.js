...
;(function($) {
	...
	$.fn.iSimulateSelect = function(iSet) {
		...
		return this.each(function() {
			...
			//i_selectbox
			//当前模拟select初始值元素
			thisCurVal = thisBox.find('.' + iSet.curSCls);//i_currentselected
			//i_currentselected
			//当前模拟select列表
			...
			*/
			...
			thisSelect.find('dd').click(function() {
				...
				//
				cIndex = thisSelect.find('dd').index(this);//this 是当前被选中的dd对象
				thisCurVal.text($(this).text());
				$(self).find('option').removeAttr('selected');
				$(self).find('option').eq(cIndex).attr('selected', 'selected');
				$('.' + iSet.selectBoxCls).css('zIndex', iSet.zindex);
				thisSelect.hide();
				var x = document.getElementById("navigatorForm");//navigatorForm是翻页用的FORM  
