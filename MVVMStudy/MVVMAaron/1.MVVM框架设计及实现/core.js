...
(function() {
  ...
  MVVM.define = function(name, factory) {  // factory 就是定义变量的函数
    var scope = {};
    //收集所有定义 //收集所有变量的值
    factory(scope);
    ...
    //model 是经过我处理后的对象 一些key已经重写了 set  get 方法  还有 one publish 等事件处理函数



    stopRepeatAssign = true;
    factory(model); //factory是用户定义的函数 就是给变量赋值的函数
    stopRepeatAssign = false;

    model.$id = name;
    return VMODELS[name] = model; //本例中 name是 'box'  model就是处理后的对象
  };
  ...
  function modelFactory(scope) {
    ...
    var normalProperties = {} //普通属性  //比如click属性  因为它的值是函数
    ...
    //转成访问控制器
    //withValue 得到description  比如 value writeable 之类
    //本例中 需要监控的变量只有 w 这一个 所以 accessingProperties 就是 {w: (newValue), __proto__: Object}
    vModel = Object.defineProperties(vModel, withValue(accessingProperties));  //Object.defineProperty(O,Prop,descriptor)

    //没有转化的函数,混入到新的vm对象中
    //click就放入到这里  click属性的get set没有做改动  因为这是一个不被watch的变量
    for (var name in normalProperties) {
      vModel[name] = normalProperties[name]
    }
    //本例中 到此 vModel有两个key  一个w 一个click  w的get set被重写
    watchProperties.vModel = vModel;
    aaObserver.call(vModel); //赋予自定义事件能力
    //经过 aaObserver之后 vModel有了 one(one) publish(trigger) remove(off) subscrible(on) 方法

    vModel.$id = generateID()
    vModel.$accessors = accessingProperties //accessingProperties是要存贮监控的变量 value是一个函数 该函数就是get set方法 同时这个函数还有一个属性  就是最顶部的subscribers
  ...
  //转成访问控制器
  //set or get
  // normalProperties 是存放不监控的变量  比如函数
  // accessingProperties 存放需要监控的变量  key是变量名  value是这个key的 get set方法

  //本例子中第一个传入的值是 name 为 "w"  val是150  其他的是 {}
  //第二次进入name是click   val 是一个函数  originalM...{} accessingP..{} 各有一个w属性(上次赋值)  其他是 {}
  function resolveAccess(name, val, originalModel, normalProperties, accessingProperties, watchProperties) {

    //缓存原始值
    originalModel[name] = val

    var valueType = $.type(val); // $.type 是JQ的方法  判断元素的类型
    ...
    //保存监控处理
    accessingProperties[name] = accessor   // name是 'w'  accessor 是一个函数
  }
  ...
  //生成求值函数与
  //视图刷新函数
  //code是属性值  scopes就是vModel  data是一个属性对象
  function parseExpr(code, scopes, data) {
    ...
    if (dataType === 'click') {
      ...
      var code = "\nreturn " + data.value + ";";
      // 创建一个匿名函数
      // Function('console.log("A")') 得到 function (function anonymous() {console.log("a") });
      var fn = Function.apply(noop, [name].concat("'use strict';\n" + prefix + ";" + code))
    }
    //生成求值函数
    data.evaluator = fn;
  ...
  // 目前所有的函数处理  css是根据传入的值改变css的属性  click是执行
  var bindingHandlers = {
